...
<body>
  ...
  <main id="main" class="main">
    ...
    <div class="pagetitle">
      <h1>Terminaed Account Page</h1>
      <nav>
        <ol class="breadcrumb">
          <li class="breadcrumb-item"><a href="/iBalay.com/iBalay-saso/index.php">Home</a></li>
          <li class="breadcrumb-item active">Terminated Account Lists</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
    ...
    <section class="section dashboard">
      ...
      $sqlOwners = "SELECT owner_id, name, contact_number, email, location, warnings, close_account FROM owners WHERE close_account = 1";
                ...
                <tbody>
                  ...
                  ?>
                    <tr>
                      ...
                      <td>Terminated</td>
                      <td>
                        <button class="btn btn-primary btn-unterminate" data-owner-id="<?php echo $rowOwner['owner_id']; ?>" data-bs-toggle="modal" data-bs-target="#confirmModal">Unterminate Account</button>
                      </td>
                    </tr>
                  <?php
                  }
...
<script>
    $(document).ready(function () {
        var ownerIdToUnterminate;

        // Function to capture owner ID when modal is shown
        $('#confirmModal').on('show.bs.modal', function (event) {
            var button = $(event.relatedTarget); // Button that triggered the modal
            ownerIdToUnterminate = button.data('owner-id'); // Extract info from data-* attributes
        });

        $('#confirmUnterminate').click(function () {
            if (ownerIdToUnterminate) {
                $.ajax({
                    url: 'unterminate_owner.php',
                    method: 'POST',
                    data: { ownerId: ownerIdToUnterminate }, // Send the owner ID to the server
