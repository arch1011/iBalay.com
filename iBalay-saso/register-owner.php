...
// Fetch owners with approval status 0 from the database
$query = "SELECT * FROM owners WHERE approval_status = 0";
...
?>
  ...
  <main id="main" class="main">
    <div class="pagetitle">
      <h1>Pending Landlord Account Request</h1>
      <nav>
        <ol class="breadcrumb">
    ...
    <section class="section">
      <div class="row">
        <div class="col-lg-12">
          ...
          <div class="card">
            <div class="card-body">
              <h5 class="card-title">Landlords Account Pending</h5>
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    // View Documents Icon
    echo '<button class="btn btn-primary" onclick="viewDocuments(' . $row['owner_id'] . ')">';
    ...
    // Approve Icon
    echo '<button class="btn btn-success" onclick="confirmApprove(' . $row['owner_id'] . ')">';
    ...
    echo '<span style="margin-right: 5px;"></span>';
        ...
        // Disapprove Icon
    echo '<button class="btn btn-danger" onclick="confirmDisapprove(' . $row['owner_id'] . ')">';
...
<script>
    ...
    function confirmAction(ownerId, action) {
        $.ajax({
            type: 'POST',
    ...
    // Function to handle confirming approval
    function confirmApprove(ownerId) {
        $('#confirmationMessage').text('Are you sure you want to approve this owner?');
        $('#confirmAction').attr('onclick', 'confirmAction(' + ownerId + ', "approve")');
        $('#confirmationModal').modal('show');
    }

    // Function to handle confirming disapproval
    function confirmDisapprove(ownerId) {
        $('#confirmationMessage').text('Are you sure you want to disapprove this owner?');
        $('#confirmAction').attr('onclick', 'confirmAction(' + ownerId + ', "disapprove")');
        $('#confirmationModal').modal('show');
    }
</script>
